...
**Model selection under `--codex`:** under `--codex` the `--task-model` / `--review-model` CLI flags override `codex_model` / `codex_reasoning_effort` — codex runs a single executor for task, review, and finalize, so `--task-model` wins and `--review-model` is the fallback; only the populated half of a `model[:effort]` spec is applied, and the `max` effort level (claude-only) is warned about and ignored. The config-file equivalents `task_model` / `review_model` are *not* bridged to codex — they feed only the claude executor. Codex model selection follows this order: the `--task-model` / `--review-model` CLI flags take precedence, then `codex_model` / `codex_reasoning_effort` in ralphex config (default `gpt-5.5` / `xhigh`) are applied as `-c` overrides to the codex CLI; set either to an empty value (e.g. `codex_model =`) in your user config to inherit that field from `~/.codex/config.toml` instead. Commenting the line out keeps the embedded default. The startup banner under `--codex` reflects whatever the codex executor will actually receive (i.e. `cfg.AppConfig.CodexModel` / `CodexReasoningEffort`).
