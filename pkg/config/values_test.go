...
func TestValuesLoader_Load_GitBackend(t *testing.T) {
	tests := []struct {
		name     string
		config   string
		expected string
	}{
		{name: "internal", config: "git_backend = internal", expected: "internal"},
		{name: "external", config: "git_backend = external", expected: "external"},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, tc.expected, values.GitBackend)
		})
	}
}

func TestValuesLoader_Load_GitBackend_NotSetFallsToEmpty(t *testing.T) {
	// embedded defaults have git_backend commented out, so parsed value should be empty
	loader := newValuesLoader(defaultsFS)
	values, err := loader.Load("", "")
	require.NoError(t, err)
	assert.Empty(t, values.GitBackend)
}

func TestValuesLoader_Load_GitBackend_LocalOverridesGlobal(t *testing.T) {
	tmpDir := t.TempDir()
	globalConfig := filepath.Join(tmpDir, "global")
	localConfig := filepath.Join(tmpDir, "local")

	require.NoError(t, os.WriteFile(globalConfig, []byte(`git_backend = internal`), 0o600))
	require.NoError(t, os.WriteFile(localConfig, []byte(`git_backend = external`), 0o600))

	loader := newValuesLoader(defaultsFS)
	values, err := loader.Load(localConfig, globalConfig)
	require.NoError(t, err)

	assert.Equal(t, "external", values.GitBackend)
}

func TestValues_mergeFrom_GitBackend(t *testing.T) {
	t.Run("merge git backend", func(t *testing.T) {
		dst := Values{GitBackend: "internal"}
		src := Values{GitBackend: "external"}
		dst.mergeFrom(&src)
		assert.Equal(t, "external", dst.GitBackend)
	})

	t.Run("empty source doesn't overwrite git backend", func(t *testing.T) {
		dst := Values{GitBackend: "internal"}
		src := Values{GitBackend: ""}
		dst.mergeFrom(&src)
		assert.Equal(t, "internal", dst.GitBackend)
	})
}
