...
// patterns is a slice of pre-compiled task-header regexes. If empty,
// DefaultHeaderPatterns() is used.
func ParsePlan(content string, patterns []*regexp.Regexp) (*Plan, error) {
	compiled := patterns
	if len(compiled) == 0 {
		compiled = DefaultHeaderPatterns()
	}
	...
	for scanner.Scan() {
		...
		// runs before the H1 title capture so a custom H1 task regex like
		// "^# (\d+)\. (.*)$" is not silently consumed as the plan title.
...
// patterns is a slice of pre-compiled task-header regexes. If empty,
// DefaultHeaderPatterns() is used.
func ParsePlanFile(path string, patterns []*regexp.Regexp) (*Plan, error) {
