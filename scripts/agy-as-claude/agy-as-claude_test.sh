#!/usr/bin/env bash
# agy-as-claude_test.sh — tests for agy-as-claude.sh wrapper.
#
...
WRAPPER="$SCRIPT_DIR/agy-as-claude.sh"
...
# create a mock agy script that emits predefined lines
create_mock_agy() {
    local mock_script="$TMPDIR_TEST/agy"
    cat > "$mock_script" << 'MOCK_EOF'
#!/usr/bin/env bash
# mock agy: emit events based on env var MOCK_EVENTS or args
...
mock_agy=$(create_mock_agy)

echo "running agy-as-claude.sh tests"
echo ""
...
output=$(MOCK_STDOUT_FILE="$TMPDIR_TEST/signal_events.txt" \
    ...
    bash "$WRAPPER" -p "test prompt" < /dev/null 2>/dev/null)
...
output=$(MOCK_STDOUT_FILE="$TMPDIR_TEST/review_events.txt" \
    PATH="$TMPDIR_TEST:$PATH" \
    bash "$WRAPPER" -p "review prompt <<<RALPHEX:REVIEW_DONE>>>" < /dev/null 2>/dev/null)
...
# create a mock agy that writes its PID and sleeps
cat > "$TMPDIR_TEST/agy_slow" << 'SLOW_EOF'
#!/usr/bin/env bash
echo $$ > "$TMPDIR_TEST/agy_pid"
echo 'starting...'
sleep 30
SLOW_EOF
chmod +x "$TMPDIR_TEST/agy_slow"

# copy the mock as "agy" for this test
cp "$TMPDIR_TEST/agy_slow" "$TMPDIR_TEST/agy"

# run wrapper in background, send SIGTERM after agy starts
PATH="$TMPDIR_TEST:$PATH" TMPDIR_TEST="$TMPDIR_TEST" \
    bash "$WRAPPER" -p "slow prompt" < /dev/null >"$TMPDIR_TEST/sigterm_output" 2>&1 &
wrapper_pid=$!

# wait for agy to write its PID (up to 3 seconds)
for i in $(seq 1 30); do
    if [[ -f "$TMPDIR_TEST/agy_pid" ]]; then
        break
    fi
    sleep 0.1
done
...
if [[ -f "$TMPDIR_TEST/agy_pid" ]]; then
    agy_child_pid=$(cat "$TMPDIR_TEST/agy_pid")
    # send SIGTERM to the wrapper
    kill -TERM "$wrapper_pid" 2>/dev/null || true
    sleep 0.5

    # check if agy child process was also terminated
    if kill -0 "$agy_child_pid" 2>/dev/null; then
        # still running — give it a moment more
        sleep 1
        if kill -0 "$agy_child_pid" 2>/dev/null; then
            fail "agy child not terminated after SIGTERM" "child PID $agy_child_pid still running"
            kill -9 "$agy_child_pid" 2>/dev/null || true
        else
            pass "SIGTERM forwarded to agy child process"
        fi
    else
        pass "SIGTERM forwarded to agy child process"
    fi
    ...
    fail "could not detect agy child PID" "pid file not created"
fi

# clean up any remaining processes
wait "$wrapper_pid" 2>/dev/null || true
rm -f "$TMPDIR_TEST/agy_pid"

# restore standard mock agy after SIGTERM test
...
output=$(MOCK_STDOUT_FILE="$TMPDIR_TEST/minimal_events.txt" \
    PATH="$TMPDIR_TEST:$PATH" \
    bash "$WRAPPER" --dangerously-skip-permissions --output-format stream-json --verbose -p "test prompt" < /dev/null 2>/dev/null)
...
output=$(MOCK_STDOUT_FILE="$TMPDIR_TEST/minimal_events.txt" \
    PATH="$TMPDIR_TEST:$PATH" \
    bash "$WRAPPER" -p "$large_prompt" < /dev/null 2>/dev/null)
...
# ---------------------------------------------------------------------------
# test: environment unsetting (prevent deadlock)
# ---------------------------------------------------------------------------
echo "test: environment unsetting"

# create a mock agy that outputs environment variables it has
cat > "$TMPDIR_TEST/agy" << 'ENV_MOCK_EOF'
#!/usr/bin/env bash
env | grep "^ANTIGRAVITY_" > "$TMPDIR_TEST/captured_env" || true
exit 0
ENV_MOCK_EOF
...
if [[ -f "$TMPDIR_TEST/captured_env" ]]; then
    captured_vars=$(cat "$TMPDIR_TEST/captured_env")
    if [[ -n "$captured_vars" ]]; then
        fail "ANTIGRAVITY_* variables were not unset" "captured variables: $captured_vars"
    else
        pass "ANTIGRAVITY_* variables successfully unset inside the wrapper"
    fi
else
    fail "could not run env check on mock agy"
fi
...
cat > "$TMPDIR_TEST/agy" << 'ADAPTER_MOCK_EOF'
#!/usr/bin/env bash
...
MOCK_STDOUT_FILE="$TMPDIR_TEST/review_events.txt" \
    PATH="$TMPDIR_TEST:$PATH" TMPDIR_TEST="$TMPDIR_TEST" \
    bash "$WRAPPER" -p "review prompt <<<RALPHEX:REVIEW_DONE>>>" < /dev/null >/dev/null 2>&1

if [[ -f "$TMPDIR_TEST/captured_prompt" ]]; then
    captured=$(cat "$TMPDIR_TEST/captured_prompt")
    if echo "$captured" | grep -q 'Ralphex review adapter for Antigravity'; then
        pass "review adapter prepended to review prompt"
    else
        fail "review adapter not prepended" "prompt: $captured"
    fi
else
    fail "could not capture prompt sent to agy"
fi
...
MOCK_STDOUT_FILE="$TMPDIR_TEST/minimal_events.txt" \
    PATH="$TMPDIR_TEST:$PATH" TMPDIR_TEST="$TMPDIR_TEST" \
    bash "$WRAPPER" -p "regular task prompt" < /dev/null >/dev/null 2>&1
...
set +e
# create a restricted PATH with only required tools, excluding agy.
no_agy_bin="$TMPDIR_TEST/no_agy_bin"
mkdir -p "$no_agy_bin"
for tool in jq bash mktemp mkfifo cat rm kill env; do
    tool_path=$(command -v "$tool" 2>/dev/null) && ln -sf "$tool_path" "$no_agy_bin/$tool"
done
PATH="$no_agy_bin" bash "$WRAPPER" -p "test prompt" < /dev/null 2>"$TMPDIR_TEST/no_agy_err"
no_agy_exit=$?
rm -r "$no_agy_bin"
set -e

if [[ $no_agy_exit -ne 0 ]]; then
    pass "exits non-zero when agy not found"
else
    fail "should exit non-zero when agy not found" "got exit code 0"
fi

if grep -q "agy is required" "$TMPDIR_TEST/no_agy_err"; then
    pass "error message mentions agy requirement"
else
    fail "no error about missing agy" "stderr: $(cat "$TMPDIR_TEST/no_agy_err")"
fi
