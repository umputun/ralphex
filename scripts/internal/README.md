...
- **update-plugin-version.sh** - Updates the two Claude metadata versions and the Codex plugin manifest version from one release tag. The Codex marketplace has no separate version. Called by goreleaser as a pre-release hook.
