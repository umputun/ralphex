...
- `idle_timeout` config / `--idle-timeout` CLI flag kills executor sessions when no output is received for a specified duration (e.g., `5m`). Unlike session timeout (fixed wall-clock limit), idle timeout resets on each output line and only fires when the session goes silent. Implemented in both `ClaudeExecutor.Run()` and `CodexExecutor.Run()` via `time.AfterFunc` with closure-based timer reset. Applies to claude and codex, including external codex review; custom external review is not affected. Disabled by default (empty/0)
...
- `idle_timeout` config option: kills claude/codex executor sessions when no output for specified duration (e.g., "5m"). Resets on each output line, only fires when session goes silent. Applies to external codex review too; custom external review is not affected. CLI flag `--idle-timeout` takes precedence. Disabled by default
