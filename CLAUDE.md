...
- `--codex` flag is an executor switch (not a new pipeline mode): sets `cfg.Executor = config.ExecutorCodex` so task, first review, second review, and finalize all run through `CodexExecutor`(s) with `MultiAgent=true` (enables `features.multi_agent` + registers the `reviewer` agent for spawn_agent calls in any phase). Forces `cfg.ExternalReviewTool = "none"` because codex-reviewing-codex is a same-model self-review with weak signal. `--pass-claude-md` (valid with the codex executor, either `--codex` or `executor = codex`) additionally sets `CodexExecutor.PassClaudeMd = true`. The pipeline `Mode` enum is unchanged; the `Executors` struct uses role-named fields (`Task`/`Review`/`External`/`Custom`), and under `--codex` `buildCodexExecutors` wires one codex instance into both `Task` and `Review` when the resolved review model/effort matches task, or two distinct instances when they differ (see the `--task-model`/`--review-model` bullet). Review prompts are shared with claude — the `{{agent:<name>}}` expansion in `pkg/processor/prompts.go` reads `cfg.AppConfig.Executor` and produces `Use the Task tool` text for claude or `spawn_agent(agent='reviewer', task='...')` for codex. Codex-specific orchestration directives (fork_context guard + wait_agent stall retry) live separately in the section-level `codexReviewGuidance` block prepended to review prompts by `prependCodexReviewGuidance` — see next bullet. Codex configuration is passed as additive `-c` flag overrides per invocation by `(*CodexExecutor).configOverrides()` in `pkg/executor/codex.go` (e.g., `-c features.multi_agent=true`, `-c agents.reviewer.description=...`, and `-c project_doc_fallback_filenames=["CLAUDE.md"]` when `--pass-claude-md`). The `-c` overrides layer on top of the user's `~/.codex/config.toml` rather than replacing it, so user customizations (model, sandbox, MCP servers) are preserved. ralphex never writes to the user's `~/.codex/` directory; for user-level CLAUDE.md content the runner prints a one-time hint suggesting `ln -s ~/.claude/CLAUDE.md ~/.codex/AGENTS.md` and the user opts in by running the command themselves
- Codex review-phase orchestration directives: `prependCodexReviewGuidance` (`pkg/processor/prompts.go`) injects a section-level `=== Codex orchestration directives ===` block at the top of `ReviewFirstPrompt` and `ReviewSecondPrompt` whenever `cfg.isCodexExecutor()` is true (no-op for claude). The block covers two codex multi_agent quirks: (a) spawn_agent must pass only `agent` and `task` — `fork_context=true` paired with explicit `agent_type` is rejected by codex's API, costing ~5 wasted retry round-trips per review iteration; (b) when wait_agent returns `{"status":{}, "timed_out":true}` for a sub-agent that died mid-tool-call, the parent must re-spawn that agent ONCE and then proceed with partial results — without this directive codex sits through four 10-min wait windows (~40 min) before deciding to retry on its own. Section-level injection works for users with embedded defaults AND users with customized review prompts that hard-code agent lists inline (bypassing the `{{agent:NAME}}` expander). Wired at three call sites in `pkg/processor/runner.go`: first review (full pipeline + review-only mode) and the review-second loop.
- Codex task-phase skill-conflict directive: `prependCodexTaskGuidance` (`pkg/processor/prompts.go`) injects the `=== Codex task-execution directives ===` block (`codexTaskGuidance`) at the top of `TaskPrompt` whenever `cfg.isCodexExecutor()` is true (no-op for claude). Wired at one call site — `runTaskPhase` in `pkg/processor/runner.go`. Rationale: codex auto-activates skills from the user's `~/.codex/skills/` based on prompt content; a user's plan-execution skill triggers on the same wording ralphex's `task.txt` uses, then runs a competing workflow (VCS detection, branch creation, task-section selection) and floods the progress stream with recited skill text. The directive tells codex that ralphex's task prompt is authoritative and a conflicting skill workflow must not be followed. Deliberately generic — names no specific skill, since installed skills vary per machine. It is a soft mitigation (a prompt instruction codex can still ignore), not a hard guard; codex 0.133.0 exposes no per-invocation skill-disable flag. Task-phase only — review phases use `prependCodexReviewGuidance`, finalize has no equivalent
- Codex output streaming: codex CLI has no `stream-json` equivalent to claude's structured output, so assistant message text + tool dispatch are only written to its session rollout file at `~/.codex/sessions/<y>/<m>/<d>/rollout-<ts>-<session-id>.jsonl`. `CodexExecutor.Run` extracts the session id from the stderr header banner (via `extractSessionID` + a buffered `sessionIDCh`) and spawns `tailRolloutFile` to follow the JSONL file like `tail -f`. `formatRolloutEvent` forwards only assistant message text — reasoning records are already covered by the stderr bold-summary stream, and all `function_call` records (`exec_command`, `spawn_agent`) are skipped as tool-machinery noise the assistant narrative already covers. `tailCtx` is canceled after stdout EOF so the tailer drains the file one more time and exits without blocking shutdown
- Codex stderr filtering: `shouldDisplay` (`pkg/executor/codex.go`) suppresses the per-iteration startup banner (separators + workdir/provider/approval/session-id) to match claude's empty-banner UX, but on the very first `Run()` call across the executor's lifetime (`headerEmitted atomic.Bool` CompareAndSwap) it whitelists three lines from the header block — `model:`, `sandbox:`, `reasoning effort:` — so users see what codex actually resolved from `~/.codex/config.toml`. Subsequent invocations get full suppression. Bold reasoning summaries (`**Considering X**`) always flow through. The ralphex-side startup banner (`printExecutorInfo` in `cmd/ralphex/main.go`) emits `sandbox:` (and `model:` / `reasoning effort:` when `codex_model` / `codex_reasoning_effort` are set in ralphex config — empty values are skipped so codex's resolved header still surfaces them) so users see ralphex's contribution before any codex output
...
- `--task-model` flag sets model for task execution with optional effort via `model[:effort]` syntax (e.g., `--task-model=opus`, `--task-model=opus:high`, `--task-model=:medium` for effort-only). Effort levels: `low`, `medium`, `high`, `xhigh`, `max`. `--review-model` sets model/effort for review phases (falls back to `--task-model`). Injected as `--model <value>` and/or `--effort <value>` into the configured `claude_command`; custom wrappers may ignore (default behavior via `*) shift ;;`) or map them to their own selection. Under the codex executor (`--codex` or `executor = codex`), `--task-model`/`--review-model` (and their config equivalents `task_model`/`review_model`) select the model/effort per phase, resolved against `codex_model`/`codex_reasoning_effort` as the codex defaults: `(Config).buildCodexExecutors` (pkg/processor/runner.go) builds a separate review `CodexExecutor` when the resolved review model/effort differs from task, otherwise the task executor handles task, review, and finalize. `ResolveCodexModelEffort` (pkg/processor/runner.go) resolves a `model[:effort]` spec against the codex defaults — an empty spec keeps the defaults, each populated half overrides its default. The `max` effort level does not exist in codex — a spec requesting it keeps the default effort and `maxDropped` is reported; the startup banner emits the warning (`codexModelBanner` in `cmd/ralphex/main.go`). The banner shows a `review model`/`review reasoning effort` line only when the review phase resolves differently from task
- `--wait` flag enables rate limit retry with specified duration (e.g., `--wait=1h`)
- `--session-timeout` flag sets per-session timeout (e.g., `--session-timeout=30m`), kills hanging sessions. Applies to claude in default executor mode and to every executor call under `--codex`; external codex/custom review in Claude mode is not affected
- `--idle-timeout` flag kills claude/codex executor sessions when no output is received for a specified duration (e.g., `--idle-timeout=5m`), resets on each output line
...
- `session_timeout` config / `--session-timeout` CLI flag sets per-session timeout. In default Claude executor mode (no `--codex`) it applies only to claude task/review/eval calls — external codex/custom review is NOT subject to the timeout (preserves existing behavior). Under `--codex`, it applies to every executor call (task, review, finalize, evaluation). When a session exceeds the timeout, it is killed and the phase loop continues to the next iteration. Applied in `runWithSessionTimeout` via `context.WithTimeout`; gated on `Executor==ExecutorCodex || toolName=="claude"`. Disabled by default (empty/0)
- `idle_timeout` config / `--idle-timeout` CLI flag kills executor sessions when no output is received for a specified duration (e.g., `5m`). Unlike session timeout (fixed wall-clock limit), idle timeout resets on each output line and only fires when the session goes silent. Implemented in both `ClaudeExecutor.Run()` and `CodexExecutor.Run()` via `time.AfterFunc` with closure-based timer reset. Wired by `buildCodexExecutor` for first-class --codex; intentionally NOT wired by `buildExternalCodexExecutor` (external codex review in default-claude mode keeps master semantics so users with `idle_timeout` configured for claude don't see new early-terminations on the external review phase). Custom external review is also not affected. Disabled by default (empty/0)
...
- `task_model` config option: model[:effort] for task execution (e.g., `opus`, `opus:high`, `:medium`). Effort values: `low`, `medium`, `high`, `xhigh`, `max`. CLI flag `--task-model` takes precedence. Parsed in `ParseModelEffort` (pkg/processor/runner.go), split on first colon. Appended to `claude_command` as `--model <m>` and/or `--effort <e>`; custom wrappers may ignore or implement the flags. Under the codex executor it instead selects the codex task-phase model/effort, resolved against `codex_model`/`codex_reasoning_effort` (see the `--task-model` key-pattern bullet). Disabled by default (empty = Claude CLI defaults)
- `review_model` config option: model[:effort] for review phases. Falls back to `task_model` if empty. CLI flag `--review-model` takes precedence. Same wrapper behavior and syntax as `task_model`; under the codex executor it selects the codex review-phase model/effort. Disabled by default
...
- `session_timeout` config option: per-session timeout (e.g., "30m", "1h"). Applies to claude in default executor mode and to every executor call under `--codex` (task/review/finalize/eval). External codex/custom review in Claude mode is NOT affected (preserves existing behavior). Kills hanging sessions and continues to next iteration. CLI flag `--session-timeout` takes precedence. Disabled by default
- `idle_timeout` config option: kills claude/codex executor sessions when no output for specified duration (e.g., "5m"). Resets on each output line, only fires when session goes silent. Applies to claude in default mode and to every executor call under `--codex`; external codex review in default-claude mode is NOT affected (preserves master behavior). Custom external review is also not affected. CLI flag `--idle-timeout` takes precedence. Disabled by default
