...
- Codex stderr filtering: `shouldDisplay` (`pkg/executor/codex.go`) suppresses the per-iteration startup banner (separators + workdir/provider/approval/session-id) to match claude's empty-banner UX, but on the very first `Run()` call across the executor's lifetime (`headerEmitted atomic.Bool` CompareAndSwap) it whitelists three lines from the header block — `model:`, `sandbox:`, `reasoning effort:` — so users see what codex actually resolved from `~/.codex/config.toml`. Subsequent invocations get full suppression. Bold reasoning summaries (`**Considering X**`) always flow through. The ralphex-side startup banner (`printExecutorInfo` in `cmd/ralphex/main.go`) emits `sandbox:` (and `model:` / `reasoning effort:` when ralphex explicitly overrode them via `--task-model`) so users see ralphex's contribution before any codex output
