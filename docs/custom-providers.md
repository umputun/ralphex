...
- ralphex selects codex-specific review prompts (`review_first_codex.txt`, `review_second_codex.txt`) that use codex `spawn_agent` / `wait_agent` vocabulary instead of Claude's Task tool syntax.
...
### Prompt customization split

A user with a customized `~/.config/ralphex/prompts/review_first.txt` (claude-mode customization) will NOT have that customization applied under `--codex` because the codex path reads a different basename. To customize the codex review prompts, create `review_first_codex.txt` and `review_second_codex.txt` alongside the claude variants in `~/.config/ralphex/prompts/` (or `.ralphex/prompts/`). The same loading priority applies: local → global → embedded default.
